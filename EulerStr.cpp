...
#include "Solver/GenerateEulerStr/GenerateEulerStr.h"
...
int main(int argc, char* argv[]){
  ...
  while (getline(infile, line))
  {
    ...
    for (auto str : Regex_list){
      ...
      auto ren = solverbin::Parser(str, false);
      ReList.emplace_back(ren.Re);
      auto EulerStr = solverbin::GenerateEulerStr(ren.Re, std::stoi(argv[3]), argv[2], std::stoi(argv[4]), std::stoi(argv[5]), std::stoi(argv[6]), 0);
      EulerStr.RegexFile = argv[1];
      EulerStr.Regex = line;
