...
conn = sqlite3.connect("/home/HybridAlgSolver/Datasets/DataBase/Outputdb/corpus.db")
cursor = conn.cursor()
...
for toolname in baselines:
    ...
    if toolname == 'GREWIA':
        ...
        FP = redos_cnt_1 - has_redos_cnt_1
        TN = GT_total_cnt - GT_has_redos_cnt - FP
        ACC = (TP + TN) / (TP + TN + FP + FN) if (TP + TN + FP + FN) > 0 else 0
        Recall = TP / (TP + FN) if (TP + FN) > 0 else 0
        Pre = TP / (TP + FP) if (TP + FP) > 0 else 0
        F1 = 2 * Pre * Recall / (Pre + Recall) if (Pre + Recall) > 0 else 0
        print(f"RMGNN - TP: {TP}, TN: {TN}, FP: {FP}, FN: {FN}, ACC: {ACC:.4f}, Recall: {Recall:.4f}, Precision {Pre:.4f}, F1: {F1:.4f}")   
    print(f"{toolname}: reported vulns (is_redos=1) = {has_redos_cnt}") 
    TP = has_redos_cnt
    ...
    Pre = TP / (TP + FP) if (TP + FP) > 0 else 0
    F1 = 2 * Pre * Recall / (Pre + Recall) if (Pre + Recall) > 0 else 0
    print(f"{toolname} - TP: {TP}, TN: {TN}, FP: {FP}, FN: {FN}, ACC: {ACC:.4f}, Recall: {Recall:.4f}, Precision {Pre:.4f}, F1: {F1:.4f}")   
