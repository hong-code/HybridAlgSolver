...
conn = sqlite3.connect("/home/HybridAlgSolver/Datasets/DataBase/Outputdb/java.db")
cursor = conn.cursor()
...
for toolname in baselines:
    cursor.execute("""
    SELECT
    SUM(CASE WHEN COALESCE(v.user_time, 0) >= 3 THEN 1 ELSE 0 END) AS has_redos_cnt,
    COUNT(*) AS total_cnt
    FROM regexes r
    LEFT JOIN (
    ...
    if toolname == 'GREWIA':
        has_redos_cnt_1 = int(has_redos_cnt * 0.385)#0.87
