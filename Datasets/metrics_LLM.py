...
def main():
    ...
    ap.add_argument(
        "--glm",
        default="/home/HybridAlgSolver/Datasets/LLMs/gpt-oss-20B_pypi.csv",
        help="Prediction CSV path (per-tool).",
    )
    ap.add_argument(
        "--corpus",
        default="/home/HybridAlgSolver/Datasets/DataBase/PyPI_regex_with_redos.csv",
    ...
    # 先把 prediction 聚合去重，避免 merge 扩行
    glm_small["pred"] = to_bool_series(glm_small["pred"])
    glm_small = (
        glm_small.groupby("decoded_regex", as_index=False)["pred"]
        .max()
    )
    ...
    # 在 merged 上直接生成 y_true / y_pred，长度一致
    merged["y_true"] = to_bool_series(merged[args.corpus_label_col])
    merged["y_pred"] = to_bool_series(merged["pred"])

    print("\n[Debug] Ground-truth raw label distribution:")
    print(merged[args.corpus_label_col].astype(str).str.strip().value_counts(dropna=False).head(20))

    print("\n[Debug] y_true value counts:")
    print(merged["y_true"].value_counts(dropna=False))

    print("\n[Debug] pred raw distribution:")
    print(merged["pred"].astype(str).str.strip().value_counts(dropna=False).head(20))

    print("\n[Debug] y_pred value counts:")
    print(merged["y_pred"].value_counts(dropna=False))
    ...
    if args.save_merged:
        ...
        merged_out["y_pred_mod"] = y_pred_mod.values
        merged_out.to_csv(
            args.save_merged,
            index=False,
            encoding="utf-8",
